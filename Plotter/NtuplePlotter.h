...
public :
	...
	NtuplePlotter(	vector<string> IN_filetags, 
						string IN_infile_path 	= "../Ntuples/v1.0/Ntuple_", 
						string IN_infile_ext 	= ".root",
						TString IN_NtupleVersion = "" ){
		...
		if( IN_NtupleVersion != "" ){
			NtupleVersion = IN_NtupleVersion;
		} else {
			...
			for( Int_t i=0; i<tarr->GetEntries(); i++){
				...
				if( element.Contains("v") && !(element.Contains("gov") || element.Contains("longlived")) ){ // handle case when xrd redirector used for LPC space
					NtupleVersion = element;
					break;
				}
			}
		}	
		cout<<"Reading in NtupleVersion as \""<<NtupleVersion<<"\""<<endl;
	...
	void SetTreeName( string IN_treename ){
		...
		if( debug) cout<<"NtuplePlotter::SetTreeNames()"<<endl;
	...
	void SetTreeNames( vector<string> IN_treenames ){
		...
		if( debug) cout<<"NtuplePlotter::ReadFromMultipleTrees()"<<endl;
	...
	string GetFiletagTreename( string filetag, string treename){
		...
		if( debug) cout<<"NtuplePlotter::GetFiletagTreename()"<<endl;
	...
	bool GetTree(string filetag, string treename ){
		...
		if( debug) cout<<"NtuplePlotter::GetTree()"<<endl;
	...
	// -------------------------------------------------------------------------------------
	void GetTrees(){
		if( debug) cout<<"NtuplePlotter::GetTrees()"<<endl;
	...
	// -------------------------------------------------------------------------------------
	void ClearFileTrees(){
		if( debug) cout<<"NtuplePlotter::ClearFileTrees()"<<endl;
	...
	void SetOutputFileTag(string output_file_tag_tmp){
		if( debug) cout<<"NtuplePlotter::SetOutputFileTag()"<<endl;
	...
	void SetOutputDirectory(string output_directory_tmp){
		if( debug) cout<<"NtuplePlotter::SetOutputDirectory()"<<endl;
	...
	TString GetOutputFileName( PlotParams myPlotParams, string plot_type = ""){
		if( debug) cout<<"NtuplePlotter::GetOutputFileName()"<<endl;		
	...
	void SetPlots(vector<PlotParams> myPlotParamsList){
		if( debug) cout<<"NtuplePlotter::SetPlots()"<<endl;		
	...
	void SetPlots2D(vector<Hist1_Hist2> myPlot2DParamsList){
		if( debug) cout<<"NtuplePlotter::SetPlots2D()"<<endl;		
	...
	void AddPlot( PlotParams myPlotParams ){
		if( debug) cout<<"NtuplePlotter::AddPlot()"<<endl;	
	...
	void SetCuts( TCut new_cut ){
		if( debug) cout<<"NtuplePlotter::SetCuts()"<<endl;		
	...
	void AddCut( TCut new_cut ){
		if( debug) cout<<"NtuplePlotter::AddCut()"<<endl;
	...
	void SetComparisonCuts( vector<TCut> cuts_compare_input, string match_filetag_or_treename = "" ){
		...
		if( debug) cout<<"NtuplePlotter::SetComparisonCuts()"<<endl;		
	...
	void SetSelectiveCuts( string match_filetag_or_treename, TCut cut ){
		if( debug) cout<<"NtuplePlotter::SetSelectiveCut()"<<endl;
	...
	void ApplySelectiveCuts(){
		if( debug) cout<<"NtuplePlotter::ApplySelectiveCuts()"<<endl;
	...
	void StampCuts( float x=0.14, float y=0.84, float textsize=0.03, TString stamp_string="") {
		if( debug) cout<<"NtuplePlotter::StampCuts()"<<endl;		
		...
		if( stamp_string == "" )
			stamp_text.DrawLatex( x, y, Form( NtupleVersion+" %s -- "+GetBetterCutTitle(cuts_all), treename.c_str() ) );
		else 
	...
	void StampText( float x=0.14, float y=0.84, float textsize=0.03, string stamp_string="") {
		if( debug) cout<<"NtuplePlotter::StampText()"<<endl;		
	...
	TString GetBetterCutTitle( TCut cut ){
		if( debug) cout<<"NtuplePlotter::GetBetterCutTitle()"<<endl;		
	...
	TString FormatMyString( string my_string ){
		if( debug) cout<<"NtuplePlotter::FormatMyString()"<<endl;		
	...
	void SetLegendPosition( double legx1_temp, double legy1_temp, double legx2_temp, double legy2_temp ){
		if( debug) cout<<"NtuplePlotter::SetLegendPosition()"<<endl;
	...
	void SetLegendNames( vector<string> legend_names_temp ){
		if( debug) cout<<"NtuplePlotter::SetLegendNames()"<<endl;
	...
	string GetFitInfo(TH1F *h, string units = ""){
		if( debug) cout<<"NtuplePlotter::GetFitInfo()"<<endl;
	...
	void SetVariableBins( vector<Double_t> Bins_variable_temp, bool variable_binwidth_equal_temp = false ){
		if( debug) cout<<"NtuplePlotter::SetVariableBins()"<<endl;		
	...
	TH1F* GetHist1D( PlotParams myPlotParams, string filetag_treename, TCut cut_compare, string hist_tag_prepend = ""){
		if( debug) cout<<"NtuplePlotter::GetHist1D()"<<endl;		
	...
	TH2F* GetHist2D( PlotParams myPlotParams_x, PlotParams myPlotParams_y, string filetag_treename, TCut cut_compare, string hist_tag_prepend = "", double manual_shift = 0.0 ){
		if( debug) cout<<"NtuplePlotter::GetHist2D()"<<endl;		
	...
	TH3F* GetHist3D( PlotParams myPlotParams_x, PlotParams myPlotParams_y, PlotParams myPlotParams_z, string filetag_treename, TCut cut_compare, string hist_tag_prepend = ""){
		if( debug) cout<<"NtuplePlotter::GetHist3D()"<<endl;		
	...
	map<string,TH1F*> GetHists( PlotParams myPlotParams ){
		if( debug ) cout<<"NtuplePlotter::GetHists()"<<endl;
	...
	THStack* GetStackHist( map<string,TH1F*> hists, PlotParams myPlotParams ){ 
		if( debug) cout<<"NtuplePlotter::GetStackHist()"<<endl;		
	...
	THStack* GetStackRatio( map<string,TH1F*> hists, PlotParams myPlotParams, string filetag_treename_divisor = "" ){
		if( debug) cout<<"NtuplePlotter::GetStackRatio()"<<endl;		
	...
	void Plot( string plot_type = "", string filetag_treename_divisor = "", vector<string> mass_lifetime = {"higgs", "llp", "ctau"}, bool multiple = false ){
		if( debug) cout<<"NtuplePlotter::Plot()"<<endl;		
		...
		for( auto PlotParams_temp: PlotParamsList ){
			...
			StampLabel( "Simulation", 140., 0.14, 0.84, 0.045 );
				...
				for( auto hist_tag: hist_tags ){
					...
					if ((multiple == false && i > 0) || (multiple == true && i%2 == 1)) {
						...
						if(TEfficiency::CheckConsistency(*h_pass, *h_total)) {
							...
							if (plot_type == "efficiency") label_y = "Efficiency";
							...
							if (i == size(hist_tags)-1 ) {
								leg->Draw();
								StampLabel( "Simulation", 140., 0.12, 0.92, 0.05 ); 
							}

							fout->cd(); // write to a root file
							std::string temp(legend_names.at(i).c_str());
							pEff->Write(output_file_name);
						}
					}
					i += 1;
	...
	void Draw2DPlot( PlotParams myPlotParams_x, PlotParams myPlotParams_y, string filetag_treename, TCut cut_compare, int i , TCut cut_base = "") {
		if( debug) cout<<"NtuplePlotter::Draw2DPlot()"<<endl;
	...
	void Plot2D( PlotParams myPlotParams_x, PlotParams myPlotParams_y, string plot_type = "" ){
		if( debug) cout<<"NtuplePlotter::Plot2D()"<<endl;		
	...
	void PlotMany2D( string plot_type = "" ){
		if( debug) cout<<"NtuplePlotter::PlotMany2D()"<<endl;		
